<script>
  ...
  import { fetchCurrentCategories } from "$lib/stores/categories";

  let isLoading = true; // Tracks loading state for current categories

  onMount(async () => {
    console.log("onMount triggered in +layout.svelte");
    try {
      // Fetch categories for the current round
      await fetchCurrentCategories();
      console.log("Current categories fetched successfully.");
    } catch (error) {
      console.error("Error in +layout.svelte:", error);
    } finally {
