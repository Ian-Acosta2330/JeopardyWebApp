...
<script>
    ...
    let currentPlayer = 0; // Track which player's turn it is
    ...
    function handleSubmit() {
          if (userInput.trim().toLowerCase() === selectedQuestion.answer.toLowerCase()) { 
              alert("Correct!");
              updateScore(gamePlayers[currentPlayer].name, selectedQuestion.Points); // Update score if correct
          } else {
              alert("Incorrect! The correct answer is: " + selectedQuestion.answer);
              updateScore(gamePlayers[currentPlayer].name, -selectedQuestion.Points); // Deduct points if incorrect
          }
